...
model_illumination_mgcv<-gam(illumination_pmv~+s(radiation_month)+question_dummy+s(id,bs="re"),family=ocat(R=7),weights=weight, data=sf_results_all,method="REML")
...
dev.off()
visreg(model_illumination_mgcv, xvar="radiation_month", gg=F, type="conditional", overlay=T, partial=T, rug=T, jitter=T, ylab="Response",top="points",ylab="Latent variable (breaks as PMV)",xlab="Monthly surface solar radiation downwards (J/m2)")
abline(h=c(-1,0.58,1.69,3.63,4.34,5.28),col="red",lty=2) # usa os mesmos pontos de quebra indicados por summary(modelo_illumination_mgcv)
text(y=-2,x=2500000,"     A lot of lack of clarity")
text(y=-0.3,x=1600000,"Lack of clarity")
text(y=1,x=2200000,"      Slight lack of clarity")
text(y=2.5,x=1600000,"Comfortable")
text(y=4,x=2500000, "         Slight excess of light")
text(y=4.7,x=2200000, "                  Moderate excess of light")
text(y=6,x=2200000,"Too much light")
...
pmv_mgcv_mean_illumination_classes<-classify(x=pmv_mgcv_mean_illumination, rcl=c(-50,-1,0.58,1.69,3.63,4.34,5.28),include.lowest=TRUE)
...
january_mgcv_illumination_classes<-classify(x=january_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
plot(january_mgcv_illumination_classes)
m <- rbind(c(0,-3),c(1,-2), c(2, -1),c(3,0),c(4,1),c(5,2),c(6,3),c(7,4))
january_pmv_mgcv_illumination_scale<-classify(x=january_mgcv_illumination_classes,rcl=m)  
plot(january_pmv_mgcv_illumination_scale,type="classes",plg=list(legend=c("Lack of clarity","Slight lack of clarity","Comfortable","Slight excess of light","Moderate excess of Light")))
...
february_mgcv_illumination_classes<-classify(x=february_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
march_mgcv_illumination_classes<-classify(x=march_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
april_mgcv_illumination_classes<-classify(x=april_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
may_mgcv_illumination_classes<-classify(x=may_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
june_mgcv_illumination_classes<-classify(x=june_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
july_mgcv_illumination_classes<-classify(x=july_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
august_mgcv_illumination_classes<-classify(x=august_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
september_mgcv_illumination_classes<-classify(x=september_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
october_mgcv_illumination_classes<-classify(x=october_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
november_mgcv_illumination_classes<-classify(x=november_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
december_mgcv_illumination_classes<-classify(x=december_mgcv_illumination_clamp, rcl=c(-2,-1,0.58,1.69,3.63,4.34,5.28,6),include.lowest=TRUE)
...
library(khroma)
sunset <- color("sunset")
plot(pmv_mgcv_mean_illumination, col=sunset(100),background="grey55")
pmv_mgcv_mean_illumination
pmv_mgcv_mean_illumination_classes<-classify(x=pmv_mgcv_mean_illumination, rcl=c(-1,0.58,1.69,3.63,4.34,5.28),include.lowest=TRUE)
plot(pmv_mgcv_mean_illumination_classes,type="classes",plg=list(legend=c("Lack of clarity","Slight lack of clarity","Comfortable","Slight excess of light","Moderate excess of light")),col=sunset(6),background="grey55",legend="left") # Classes: 0 = Lack of clarity, 1 = Slight lack\n of clarity, 2= Comfortable, 3= Slight excess\n of light\n,  4= Moderate excess\n of light
...
library(khroma)
YlOrBr <- color("YlOrBr")
plot(ppd_mgcv_illumination_mean, col=YlOrBr(100),background="grey55")
sbar(d=2000,xy=c(-165,-65),divs=2,type="bar",below="km",ticks=T,cex=.6)
north(xy=c(-160,-45),type=2,cex=.7,label=NULL)
