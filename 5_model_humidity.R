...
#Model for humidity
model_humidity_mgcv<-gam(humidity_pmv~s(humidity_month)+s(humidity_mean,k=4)+s(temperature_month,k=4)+question_dummy+s(id,bs="re"),family=ocat(R=7),weights=weight, data=sf_results_all,method="REML")
...
dev.off()
visreg(model_humidity_mgcv, xvar="humidity_month", gg=F, type="conditional", overlay=T, partial=T, rug=T, jitter=T, ylab="Response",top="points",ylab="Latent variable (breaks as PMV)",xlab="Monthly relative humidity (%)")
abline(h=c(-1,0.11,0.89,1.95,2.9,4.07),col="red",lty=2) 
...
abline(h=c(-1,0.11,0.89,1.95,2.9,4.07),col="red",lty=2) # usa os mesmos pontos de quebra indicados por summary(modelo_humidity_mgcv)
...
visreg(model_humidity_mgcv, xvar="temperature_month", gg=F, type="conditional", overlay=T, partial=T, rug=T, jitter=T, ylab="Response",top="points",ylab="Latent variable (breaks as PMV)",xlab="Monthly temperature (Celsius)")
abline(h=c(-1,0.11,0.89,1.95,2.9,4.07),col="red",lty=2) # usa os mesmos pontos de quebra indicados por summary(modelo_humidity_mgcv)
text(y=-3,x=-20,"Very dry")
text(y=-0.3,x=-20,"Dry")
text(y=0.5,x=-20,"Slightly dry")
...
pmv_mgcv_mean_humidity_classes<-classify(x=pmv_mgcv_mean_humidity, rcl=c(-50,-1,0.45,1.63,3.51,4.51,5.49),include.lowest=TRUE)
...
global(pmv_mgcv_mean_humidity, fun="range")
#install.packages("khroma")
library(khroma)
sunset <- color("sunset")
plot(pmv_mgcv_mean_humidity, col=sunset(100),background="grey55")
pmv_mgcv_mean_humidity_classes<-classify(x=pmv_mgcv_mean_humidity, rcl=c(-50,-1,0.11,0.89,1.95,2.9,4.07),include.lowest=TRUE)
plot(pmv_mgcv_mean_humidity_classes,type="classes",plg=list(legend=c("Very dry","Dry","Slightly dry","Comfortable","Slightly humid")),col=sunset(6),background="grey55",legend="left") #Classes: 0 = very cold, 1 = cold,  2= slightly cold, 3 = comfortable, 4 = slightly hot, 5= hot, 6 = very hot
...
january_mgcv_humidity_classes<-classify(x=january_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
february_mgcv_humidity_classes<-classify(x=february_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
march_mgcv_humidity_classes<-classify(x=march_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
april_mgcv_humidity_classes<-classify(x=april_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
may_mgcv_humidity_classes<-classify(x=may_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
june_mgcv_humidity_classes<-classify(x=june_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
july_mgcv_humidity_classes<-classify(x=july_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
august_mgcv_humidity_classes<-classify(x=august_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
september_mgcv_humidity_classes<-classify(x=september_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
october_mgcv_humidity_classes<-classify(x=october_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
november_mgcv_humidity_classes<-classify(x=november_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
december_mgcv_humidity_classes<-classify(x=december_mgcv_humidity_clamp, rcl=c(-2,-1,0.11,0.89,1.95,2.9,4.07,6),include.lowest=TRUE)
...
library(khroma)
YlOrBr <- color("YlOrBr")
plot(ppd_mgcv_humidity_mean, col=YlOrBr(100),background="grey55")
sbar(d=2000,xy=c(-165,-65),divs=2,type="bar",below="km",ticks=T,cex=.6)
north(xy=c(-160,-45),type=2,cex=.7,label=NULL)
